...
namespace app.advertise.dtos.Vendor.Validators
{
    ...
    {
        public ApplicationDetailsValidator()
        {
            ...
            RuleFor(ac => ac.AppliFromDate).NotEmpty().WithMessage("From Date is required").GreaterThanOrEqualTo(ac=>DateTime.Now).WithMessage("From date must >= Today's date");
